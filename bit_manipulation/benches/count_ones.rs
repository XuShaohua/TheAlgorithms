...
pub fn brian_bench(c: &mut Criterion) {
    c.bench_function("bit::brian-25", |b| {
        b.iter(|| brian_kernighans_algorithm(black_box(25)))
    });
    c.bench_function("bit::brian-37", |b| {
        b.iter(|| brian_kernighans_algorithm(black_box(37)))
    });
    c.bench_function("bit::brian-58", |b| {
        b.iter(|| brian_kernighans_algorithm(black_box(58)))
    });
}

pub fn modulo_bench(c: &mut Criterion) {
    c.bench_function("bit::modulo-25", |b| {
        b.iter(|| modulo_operator(black_box(25)))
    });
    c.bench_function("bit::modulo-37", |b| {
        b.iter(|| modulo_operator(black_box(37)))
    });
    c.bench_function("bit::modulo-58", |b| {
        b.iter(|| modulo_operator(black_box(58)))
    });
}

pub fn builtin_bench(c: &mut Criterion) {
    c.bench_function("bit::builtin-25", |b| b.iter(|| builtin(black_box(25))));
    c.bench_function("bit::builtin-37", |b| b.iter(|| builtin(black_box(37))));
    c.bench_function("bit::builtin-58", |b| b.iter(|| builtin(black_box(58))));
}
