...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("reverse_std 20", |b| b.iter(|| (&mut arr_20).reverse()));
    ...
    c.bench_function("reverse_std 200", |b| b.iter(|| (&mut arr_200).reverse()));
    ...
    c.bench_function("reverse_std 2k", |b| b.iter(|| (&mut arr_2k).reverse()));
    ...
    c.bench_function("reverse_std 20k", |b| b.iter(|| (&mut arr_20k).reverse()));
    ...
    c.bench_function("reverse_std 200k", |b| b.iter(|| (&mut arr_200k).reverse()));
    c.bench_function("reverse_custom 200k", |b| {
        b.iter(|| reverse_array(&mut arr_200k))
    });
    drop(arr_200k);

    let mut arr_2m: Vec<i32> = (0..black_box(2000_000)).collect();
    c.bench_function("reverse_std 2m", |b| b.iter(|| (&mut arr_2m).reverse()));
